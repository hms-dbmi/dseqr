...
test_that("add_linkout skips NA entries", {
  expect_true(is.na(with_linkouts$`Pubchem CID`[3]))
})

test_that("add_linkout uses pre and post url", {
  expect_match(with_linkouts$`Pubchem CID`[1], 'https://pubchem.ncbi.nlm.nih.gov/compound/1234/')
})

test_that("add_linkout adds title with 'Go to' prepended", {
  expect_match(with_linkouts$`Pubchem CID`[1], 'title="Go to Pubchem "')
})
