...
diff_expr <- function (eset, data_dir, anal_name, contrast = 'test-ctrl', annot = "SYMBOL", svobj = list('sv' = NULL), numsv = 0, prev_fit = NULL, prev_anal = NULL) {
  ...
  if (!is.null(prev_fit)) {
    # re-use fit from previous analysis
...
#'   meta-analysis.
diff_anal <- function(eset, anal_name, contrast, data_dir, lm_fit = NULL, svobj = list(sv = NULL), numsv = 0, annot = "SYMBOL", rna_seq = TRUE){
  ...
  if (is.null(lm_fit)) lm_fit <- fit_lm(eset, mod, rna_seq)
  ebayes_sv <- fit_ebayes(lm_fit, mod, contrast)
  ...
  diff_expr <- list(pdata = pdata, top_table = top_table, ebayes_sv = ebayes_sv, annot = annot, numsv = numsv, groups = groups)
  ...
  # also save lm_fit result (slow and can re-use for other contrasts)
  fit_path <- file.path(data_dir, paste0('lm_fit_', numsv, 'svs.rds'))
...
fit_ebayes <- function(lm_fit, mod, contrasts) {
  contrast_matrix <- limma::makeContrasts(contrasts = contrasts, levels = mod)
  eb_fit <- limma::contrasts.fit(lm_fit, contrast_matrix)
...
fit_lm <- function(eset, mod, rna_seq = TRUE) {
