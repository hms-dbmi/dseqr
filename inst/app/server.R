...
scSampleGroups <- function(input, output, session, dataset_dir, resoln_dir, dataset_name, input_scseq = function()NULL, show_pbulk = function()FALSE, counts = function()NULL) {
    ...
    output$groups_table <- rhandsontable::renderRHandsontable({
        rhandsontable::rhandsontable(data = dl_meta(),
                                     width = '100%',
                                     stretchH = "all",
                                     contextMenu = FALSE,
                                     rowHeaders = FALSE,
    ...
    dl_meta <- reactive({
        ...
        dl_meta$Sample <- row.names(dl_meta)

        return(dl_meta[, c(3,1,2)])
    })
...
groupsModal <- function(session) {
    label <- "Click upload or drag files:"

    modalDialog(
        div(id=session$ns('groups_table_container'), class='handsontable-container',
            rhandsontable::rHandsontableOutput(session$ns('groups_table'), width='100%')
        ),
        title = 'Edit Group Name and Pair (optional)',
        size = 'l',
        footer = tagList(
