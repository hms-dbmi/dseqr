...
dsPage <- function(input, output, session, data_dir, sc_dir, bulk_dir, indices_dir) {
  ...
  adj_path <- reactive(file.path(data_dir, dsForm$dataset_dir(), paste0('adjusted_', dsForm$num_svs(), 'svs.rds')))
...
dsMDSplotly <- function(input, output, session, data_dir, dataset_dir, anal_name, new_anal) {
  ...
  anal <- reactive({
    ...
    # for legacy data
    if ('scaling_sva' %in% names(anal$mds)) {
      browser()
      anal$mds$scaling_adj <- anal$mds$scaling_sva
      anal$mds$scaling_sva <- NULL
      saveRDS(anal, anal_path)
    }

    if ('scaling_clean' %in% names(anal$mds)) {
      browser()
      anal$mds$scaling_adj <- anal$mds$scaling_clean
      anal$mds$scaling_clean <- NULL
      saveRDS(anal, anal_path)
    }
    return(anal)
  })
  ...
  plotly_fun <- reactive({
    mds <- anal()$mds
    plotlyMDS(scaling = mds$scaling, scaling_clean = mds$scaling_clean)
  })
...
dsFormAnal <- function(input, output, session, data_dir, sc_dir, bulk_dir, dataset_name, dataset_dir, new_anal, new_dataset, explore_eset) {
  ...
  # obdate svobj when svobj_path updates
  observe({
  ...
  observe({
    svobj <- svobj()
    updateSliderInput(session, 'num_svs', min = 0, max = svobj$n.sv)
  })
