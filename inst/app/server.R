...
drugsForm <- function(input, output, session, new_anal, data_dir, pert_query_dir) {
  ...
  genesPlotlyOptions <- callModule(genesPlotlyOptions, 'genes')
...
#' @export
#' @keywords internal
genesPlotlyOptions <- function(input, output, session, drug_study, show_genes) {


  #  toggle  showing genes options
  shiny::observe({
    toggle('genes-panel', condition = show_genes(), anim = TRUE)
  })
...
drugsGenesPlotly <- function(input, output, session, data_dir, anal, is_sc, sc_inputs, drug_study) {
...
drugsPertInput <- function(ns, type) {
